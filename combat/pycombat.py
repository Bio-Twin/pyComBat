...
def check_ref_batch(ref_batch, batch, batchmod):
    ...
    if ref_batch is not None:
        if ref_batch not in batch:
            ...
            exit(0)
        print("Using batch " + str(ref_batch) + " as a reference batch.")
...
def treat_covariates(batchmod, mod, ref, n_batch):
    ...
    print(
        "Adjusting for "
        + str(len(design) - len(np.transpose(batchmod)))
...
def pycombat(
    ...
    **kwargs,
    ...
    if not (NAs):
        ...
        bayes_data_df = pd.DataFrame(bayes_data, columns=list_samples, index=list_genes)
